...
public class EditElementDialog extends DialogFragment implements DialogInterface.OnClickListener {
    ...
    public EditElementDialog(int id, int root){
        this.id = id;
        this.root = root;
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup conteiner, Bundle savedInstance){
        ...
        switch (root){
            case (0):
                //sphere
                this.r = Integer.parseInt(dbManager.GetElement(id).get("color").toString().split(":")[0]);
                this.g = Integer.parseInt(dbManager.GetElement(id).get("color").toString().split(":")[1]);
                this.b = Integer.parseInt(dbManager.GetElement(id).get("color").toString().split(":")[2]);
        ...
        edittxt.setText(dbManager.GetElement(id).get("name").toString());
        final TextView editname = v.findViewById(R.id.dialog_name);
        editname.setText(getContext().getString(R.string.edit_name));
        Button ok = v.findViewById(R.id.ok_btn);
        ok.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (edittxt.getText().length() != 0) {
                    dbManager.UpdateElement(edittxt.getText().toString(), id, "" + r + ":" + g + ":" + b);
                    dismiss();
